...
The project uses a standard Data Science directory structure:

```text
├── data/
│   ├── raw/             # Raw collected datasets
│   ├── interim/         # Intermediate processing artifacts (ignored by git)
│   └── processed/       # Final curated datasets for modeling
├── notebooks/           # Jupyter notebooks spanning the entire pipeline
├── reports/             # HTML data profiling reports
├── .gitignore
└── README.md
```

### 1. Data Collection & Preprocessing (`notebooks/`)
*   **`data-preprocessing-flats.ipynb` & `data-preprocessing-houses.ipynb`**: Initial cleaning of the raw data.
*   **`merge-flats-and-house.ipynb`**: Combines the cleaned flats and houses datasets.
...
*   **`eda-pandas-profiling.ipynb`**: Generates a comprehensive, automated HTML profiling report summarizing data statistics.

### 3. Data Engineering & Missing Values (`notebooks/`)
...
### 4. Feature Engineering & Selection (`notebooks/`)
...
### 5. Machine Learning Modeling (`notebooks/`)
...
## Key Files
*   **`reports/output_report.html`**: The automated data profiling report.
*   **`data/processed/gurugram_properties_post_feature_selection_v2.csv`**: The final dataset used for training the machine learning models.
*   **`data/raw/latlong.csv`**: Latitude and longitude coordinates used for geospatial analysis of properties.
